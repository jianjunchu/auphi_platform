...
public class KettleDataSourceRepositoryTransDelegate extends KettleDataSourceRepositoryBaseDelegate {
  ...
  public KettleDataSourceRepositoryTransDelegate( KettleDataSourceRepository repository ) {
    super( repository );
  }
      ...
      try {
        ...
        if ( transMeta.getObjectId() != null ) {
          ...
          RepositoryAttributeInterface attributeInterface =
            new KettleDataSourceRepositoryTransAttribute( repository.connectionDelegate, transMeta.getObjectId() );
...
//    repository.connectionDelegate.getDatabase().closeInsert();
    repository.getDatabase().insertTableRow(KettleDatabaseRepository.TABLE_R_TRANSFORMATION, table);
  ...
  private synchronized ObjectId insertTransHop( ObjectId id_transformation, ObjectId id_step_from,
    ...
    repository.getDatabase().insertTableRow(KettleDatabaseRepository.TABLE_R_TRANS_HOP, table);
...
//    repository.connectionDelegate.getDatabase().closeInsert();
    ...
    repository.getDatabase().insertTableRow(KettleDatabaseRepository.TABLE_R_DEPENDENCY, table);
