...
import org.firzjb.kettle.trans.step.StepDecoder;
import org.firzjb.kettle.trans.step.StepEncoder;
import org.firzjb.kettle.trans.step.StepErrorMetaCodec;
	...
	public AbstractMeta decode(String graphXml, CurrentUserResponse user) throws Exception {
		...
		for(int i=0; i<count; i++) {
			...
			if (cell.isEdge()) {
				...
				TransHopMeta hopinf = TransHopMetaCodec.decode(transMeta, cell);//new TransHopMeta(null, null, true);
	    ...
	    if(StringUtils.hasText(maxdate)) {
	    	...
	    	transMeta.setMaxDateTable(jsonObject.optString("table"));
	    	transMeta.setMaxDateField(jsonObject.optString("field"));
