...
import org.firzjb.kettle.bean.Ext3Node;
import org.firzjb.kettle.core.row.ValueMetaAndDataCodec;
...
public class SystemMainController {
	...
	protected @ResponseBody List fileexplorer(@RequestParam String path, @RequestParam int extension, @ApiIgnore @CurrentUser CurrentUserResponse user) throws Exception {
		if(StringUtils.isEmpty(path)){
			path = org.firzjb.base.common.Const.getUserDir(user.getOrganizerId());
		}
			...
			if(files != null) {
				for(File file : files) {
					...
					if(file.isDirectory()) {
						...
						node.setShowName(org.firzjb.base.common.Const.getUserFilePath(user.getOrganizerId(),file.getAbsolutePath()));
