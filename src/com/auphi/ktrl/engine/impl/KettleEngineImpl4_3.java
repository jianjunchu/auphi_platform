...
*/
        ...
        boolean connected = false;
        try {
            rep = (Repository) getRep(repName);
            connected = true;


            RepositoryDirectoryInterface directory = (RepositoryDirectoryInterface) getDirectory(rep, filePath);

            if(directory!=null){
                if(fileType.equalsIgnoreCase(TYPE_TRANS)){
                    TransMeta transMeta = rep.loadTransformation(fileName,directory,null,true,null);
                    success = executeTrans(transMeta, null, execType, monitorSchedule,arguments,params,variables);

                }else if(fileType.equalsIgnoreCase(TYPE_JOB)){

                   JobMeta jobMeta =  rep.loadJob(fileName,directory,null,null);
     ...
     * @throws Exception
     */
    private boolean executeTrans(TransMeta transMeta,  HashMap<?, ?> prop, int execType, MonitorScheduleBean monitorSchedule,Map<String,String> arguments, Map<String,String> params, Map<String,String> variables) throws Exception{
        boolean success = false;
        ...
        try {
            ...
            SimpleLoggingObject simpleLoggingObject = new SimpleLoggingObject("SPOON",LoggingObjectType.JOB,null);
            simpleLoggingObject.setContainerObjectId(simpleObjectId);

            trans.setParent(simpleLoggingObject);
            LogChannelInterface logChannel =  trans.getLogChannel();

            String  logChannelId = trans.getLogChannelId();
            monitorSchedule.setId_logchannel(logChannelId);
            logChannel.logMinimal("ETL--TRANS Start of run",new Object[0]);

            TransExecutor transExecutor = TransExecutor.initExecutor(monitorSchedule,trans,execType);
     ...
     * @throws Exception
     */
    public boolean executeJob(JobMeta jobMeta, Repository rep,  HashMap<?, ?> prop, int execType, MonitorScheduleBean monitorSchedule,Map<String,String> arguments, Map<String,String> params, Map<String,String> variables) throws Exception{
        boolean success = false;
        Date start = new Date();
        Date stop;
        ...
        try{
            ...
            job = new Job(rep,jobMeta,simpleLoggingObject);
            ...
            if(rep != null){
                ...
                String[] ss = {};
                job = new Job(jobMeta.getName(),jobMeta.getFilename(),ss);
            }
            ...
            logChannel.logMinimal("ETL--JOB Start of run",new Object[0]);
...
//        System.out.println(sb.toString());
    ...
    public static Connection getRepConnection(Object rep)
    {
        ...
        conn = repository.getDatabase().getConnection();
		...
		try{
            ...
            if(jobLogTableName != null){
            	...
            	while(rs.next()){
    				...
    				monitorScheduleBean.setLines_deleted(rs.getLong("LINES_REJECTED"));
...
//                        activeTransThreads.get(stop_trans).stop();
                	...
                	Date start_date =monitorBean.getStartTime();
