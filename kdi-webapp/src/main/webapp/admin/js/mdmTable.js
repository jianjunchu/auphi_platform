...
//					}
	...
	modifyMdmTableData = function (rowIndex,primary){
		...
		var id = store.getAt(rowIndex).get(primary);
	...
	function deleteData(){
		...
		var recordGrid = grid.getSelectionModel().getSelected();
		var primary_key_value = jsArray2JsString(record, 'primary_key_value');
		var primary_key_column = jsArray2JsString(record, 'primary_key_column');
		var id_table = recordGrid.data.id_table;
		Ext.Msg.confirm('请确认', '你真的要删除吗?', function(btn, text) {
			if (btn == 'yes') {
				showWaitMsg();
				Ext.Ajax.request( {
