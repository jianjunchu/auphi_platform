			...
			items : [{
				...
				items : [{
					...
					items: [{                     //这里可以为多个Item，表现出来是该列的多行
						...
						id:'id_model_main',
						fieldLabel : '模型编码', // 标签
						name : 'id_model', // name:后台根据此name属性取
						anchor:'90%',
						listeners: {
							blur: checkIdModel
						}
...
//---------------------------------------------------------------
	...
	storeAttribute.on('beforeload',function(){
		storeAttribute.baseParams = {id_model:Ext.getCmp('id_model_main').getValue()};
	});
				...
				'tabchange' : function(tab, newc, oldc) {
					if(newc.id == "id_grid_attribute"){
						...
						if (mode == 'add'){
							...
							if(checkBeforeUpdate(gridModel)){
								var id_model = getSelectModelId(gridModel);
								queryAttribute();
							}							
						}
						...
						constaintAttributeStore.load({
							params : {id_model : Ext.getCmp('id_model_main').getValue()}
						});						
...
//------------------------------------------------------------------------------------		
		function checkIdModel(){
			Ext.Ajax.request({
				url:"../mdmModel/checkIdModel.shtml", 
				success:function(response,options){
					if(response.responseText == "failure"){
						var errorMsg = "该模型编码已存在";
						Ext.MessageBox.alert('错误', errorMsg);
					}
				},
				params : {
					id_model : Ext.getCmp("id_model_main").getValue()
				}
			});
		}
...
//---------------model end---------------------------------------------------------		
		...
		function addAttribute() {
			...
			Ext.getCmp('id_model_attribute').setValue(Ext.getCmp('id_model_main').getValue());
